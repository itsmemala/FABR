...
growth=0.1
res_path="/home/local/data/ms/fabr_data/IntentSH/IntentSH_LAMAS_L2.2/${note}seed${seed}_${acc_drop_threshold}adt/IntentSH_LAMAS_t"
...
# do
	...
	# CUDA_VISIBLE_DEVICES=0 python  FABR//run.py --bert_model 'bert-base-uncased' --experiment hwu64 --approach bert_adapter_ewc_freeze --imp function --baseline ewc_freeze --backbone bert_adapter --note $note --idrandom $randid --seed $seed --scenario cil --use_rbs True --train_batch_size 128 --num_train_epochs 50 --valid_loss_es 0.002 --lr_patience 5 --learning_rate $lr --fisher_combine max --use_l2 True --l2_lamb 0.000000075 --break_after_task 0 --my_save_path ${res_path}${id}_gold.${lr_id}/ --only_mcl True
...
do
	...
	best_lr=${lr_array[$best_lr_id-1]}  # -1 for array indexing
	past_lr="$past_lr,$best_lr"
	start_lamb=5.549827275 #=0.9*prev
