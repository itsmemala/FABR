...
class Attr_Buffer:
    """
    The memory buffer of rehearsal method with attributions
    """
    def __init__(self, buffer_size, device, n_tasks=None, mode='reservoir'):
        assert mode in ['ring', 'reservoir']
        self.buffer_size = buffer_size
        ...
        if mode == 'ring':
            ...
            self.task_number = n_tasks
            self.buffer_portion_size = buffer_size // n_tasks
        self.attributes = ['examples', 'labels', 'logits', 'task_labels','segment_ids','input_mask','attributions']
    ...
    def init_tensors(self,
        ...
        :param logits: tensor containing the outputs of the network
        :param task_labels: tensor containing the task labels
        :param attributions: tensor containing input feature attributions
        """
        for attr_str in self.attributes:
            attr = eval(attr_str)
            if attr is not None and not hasattr(self, attr_str):
                ...
                setattr(self, attr_str, torch.zeros((self.buffer_size,
                        *attr.shape[1:]), dtype=typ, device=self.device))
    def add_data(self, examples, segment_ids=None, input_mask=None, labels=None, logits=None, task_labels=None, attributions=None):
        """
        Adds the data to the memory buffer according to the reservoir strategy.
        :param examples: tensor containing the images
        :param labels: tensor containing the labels
        ...
        if not hasattr(self, 'examples'):
            self.init_tensors(examples, segment_ids,input_mask, labels,logits, task_labels, attributions)
