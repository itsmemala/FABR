...
past_lr=0.0003,0.0003,0.0003,0.00003,0.0003
past_lamb=0,28.24295365,0.87279636,0.06961986,0.00555333
best_lamb=
# start_model_path="${res_path}2.34.LA_phase.1/"
start_model_path="${res_path}3.22.LA_phase.1/"
...
do
	...
	## Lamb Down
	lamb_down=0.0003 #0.001 #0.0001 #0.00001 #0.00001 #0.0001 #0.000000001
	elasticity_up_mult=0.0001 #0.0001 #0.0001 #0.0001 #0.00001 #0.0001 #0.000000001
	alpha_lamb_i=40 #34:perf=0 at best loss #33: Similar to 32 #32:Less CF using LrP=20 #31:100% CF #30:Valid loss is in expected range of 2.5 to 17 but perf=0 at best loss #29: LrP=10 works but 100% CF
	found_best=false
	while [ $found_best=false ]
	do
		((alpha_lamb_i++))
		...
		python  FABR//run.py --bert_model 'bert-base-uncased' --experiment hwu64 --approach bert_adapter_ewc_freeze --imp function --baseline ewc_freeze --backbone bert_adapter --note $note --idrandom $randid --seed $seed --scenario cil --use_rbs True --train_batch_size 512 --num_train_epochs 50 --valid_loss_es 0.002 --lr_patience 50 --custom_lr $custom_lr --custom_lamb $custom_lamb --fisher_combine max --break_after_task $id --save_alpharel True --my_save_path ${res_path}${id}.${best_lamb_i}.LA_phase.${alpha_lamb_i}/ --start_at_task $id --start_model_path $start_model_path --elasticity_down_max_lamb $elasticity_up_max_lamb --elasticity_down_mult $elasticity_up_mult --elasticity_up $lamb_down --frel_cut_type pdm --pdm_frac $pdm_frac --no_frel_cut_max $no_frel_cut_max --la_model_path $la_model_path
...
do
	...
	do
		...
		printf "\n\nLamb Iteration $custom_lamb \n\n"
		mkdir -p  ${res_path}${id}.${lamb_i}/
		python  FABR//run.py --bert_model 'bert-base-uncased' --experiment hwu64 --approach bert_adapter_ewc_freeze --imp function --baseline ewc_freeze --backbone bert_adapter --note $note --idrandom $randid --seed $seed --scenario cil --use_rbs True --train_batch_size 512 --num_train_epochs 50 --valid_loss_es 0.002 --lr_patience 50 --custom_lr $custom_lr --custom_lamb $custom_lamb --fisher_combine max --break_after_task $id --save_alpharel True --my_save_path ${res_path}${id}.${lamb_i}/ --start_at_task $id --start_model_path $start_model_path --only_mcl True
		python3 FABR/calc_next_lamb.py --my_save_path ${res_path}${id} --rand_idx $randid --seed $seed --dataset $dataset --lamb_i $lamb_i --lamb $lamb --decay $decay --acc_drop_threshold $acc_drop_threshold --tid $id
		found_best=`cat ${res_path}${id}.${lamb_i}_foundbestlamb.txt`
		python3 FABR/plot_lamb_results.py --my_save_path ${res_path}${id} --rand_idx $randid --seed $seed --dataset $dataset --lamb_i $lamb_i --lamb $lamb --acc_drop_threshold $acc_drop_threshold --tid $id
		if [ $found_best = found ]; then
