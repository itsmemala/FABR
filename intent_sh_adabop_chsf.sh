...
res_path="/home/local/data/ms/fabr_data/IntentSH/Adabop_Fixed_CSB${choose_second_best}/${note}seed${seed}/IntentSH_Adabop_t"
...
do
	...
	python  FABR//run.py --bert_model 'bert-base-uncased' --experiment hwu64 --approach bert_adapter_adabop --backbone bert_adapter --baseline adabop --note $note --idrandom $randid --seed $seed --scenario cil --use_rbs True --train_batch_size 128 --num_train_epochs 50 --eval_batch_size 128 --valid_loss_es 0.002 --lr_patience 3 --learning_rate $lr --break_after_task 0 --my_save_path ${res_path}${id}_gold.${lr_id}/
done
