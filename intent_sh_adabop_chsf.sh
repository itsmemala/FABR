...
res_path="/home/local/data/ms/fabr_data/IntentSH/Adabop/${note}seed${seed}/IntentSH_Adabop_t"
...
do
	...
	python  FABR//run.py --bert_model 'bert-base-uncased' --experiment hwu64 --approach bert_adapter_adabop --backbone bert_adapter --baseline adabop --note $note --idrandom $randid --seed $seed --scenario cil --use_rbs True --train_batch_size 128 --num_train_epochs 30 --eval_batch_size 128 --valid_loss_es 0.002 --lr_patience 3 --learning_rate $lr --break_after_task 0 --my_save_path ${res_path}${id}_gold.${lr_id}/
done
...
do
	...
	do
		...
		mkdir -p  ${res_path}${id}_gold.${lr_id}/
		python  FABR//run.py --bert_model 'bert-base-uncased' --experiment hwu64 --approach bert_adapter_adabop --backbone bert_adapter --baseline adabop --note $note --idrandom $randid --seed $seed --scenario cil --use_rbs True --train_batch_size 128 --num_train_epochs 30 --eval_batch_size 128 --valid_loss_es 0.002 --lr_patience 3 --custom_lr $custom_lr --tc_epsilon $tc_epsilon --tc_lamb_s $tc_lamb_s --tc_lamb_l $tc_lamb_l --break_after_task $id --my_save_path ${res_path}${id}_gold.${lr_id}/ --start_at_task $id --start_model_path $start_model_path
	done
