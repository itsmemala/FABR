...
past_lr=0.0003,0.00003,0.0003,0.00003,0.0003,0.00003
past_lamb=0,0.32346185,0.32346185,0.01110666,5.89036868e-10,4.82503178e-23
start_model_path="${res_path}5.287/"
...
do
	...
	do
		...
		python  FABR//run.py --bert_model 'bert-base-uncased' --experiment hwu64 --approach bert_adapter_ewc_freeze --imp function --baseline ewc_freeze --backbone bert_adapter --note $note --idrandom $randid --seed $seed --scenario cil --use_rbs True --train_batch_size 128 --num_train_epochs 50 --valid_loss_es 0.002 --lr_patience 5 --custom_lr $custom_lr --custom_lamb $custom_lamb --fisher_combine max --break_after_task $id --save_alpharel True --my_save_path ${res_path}${id}.${best_lamb_i}.LA_phase.${alpha_lamb_i}/ --start_at_task $id --start_model_path $start_model_path --elasticity_down_max_lamb $elasticity_up_max_lamb --elasticity_down_mult $elasticity_up_mult --elasticity_up $lamb_down --frel_cut_type pdm --pdm_frac $pdm_frac --no_frel_cut_max $no_frel_cut_max
		python3 FABR/calc_next_lamb_down.py --my_save_path ${res_path}${id} --rand_idx $randid --seed $seed --dataset $dataset --best_lr_id $best_lr_id --best_lamb_i $best_lamb_i --alpha_lamb_i $alpha_lamb_i --lamb_down $lamb_down --growth $growth --tid $id
...
# python3 FABR/calc_next_lamb_down.py --my_save_path /home/local/data/ms/fabr_data/IntentSH/IntentSH_LAMAS_NoL1_Custom_1.0lambup_searchlambdown_0.9pdmfrac{no_frel_cut_max}/random0seed0_0.3adt/IntentSH_LAMAS_t3 --rand_idx 0 --seed 0 --dataset hwu64 --best_lr_id 1 --best_lamb_i 33 --alpha_lamb_i 1 --lamb_down 1.0 --growth 0.9 --tid 3