...
past_lr=0.0003,0.0003,0.0003,0.0003,0.0003,0.0003 #t3: 0.00003
past_lamb=0,28.24295365,28.24295365,28.24295365,28.24295365,56
best_lamb=28.24295365
# start_model_path="${res_path}2.1.LA_phase.6/"
# start_model_path="${res_path}3.1.LA_phase.6/"
start_model_path="${res_path}4.1.LA_phase.3/"
...
# 	# 	python  FABR//run.py --bert_model 'bert-base-uncased' --experiment hwu64 --approach bert_adapter_ewc_freeze --imp function --baseline ewc_freeze --backbone bert_adapter --note $note --idrandom $randid --seed $seed --scenario cil --use_rbs True --train_batch_size 128 --num_train_epochs 50 --valid_loss_es 0.002 --lr_patience 50 --custom_lr $custom_lr --custom_lamb $custom_lamb --fisher_combine avg --break_after_task $id --save_alpharel True --my_save_path ${res_path}${id}.${lamb_i}/ --start_at_task $id --start_model_path $start_model_path --only_mcl True
...
# 	cur_pdm_frac=0.7
# 	alpha_lamb_i=18
# 	#18:lamb=285,pdm=0.7 t3 perf=70
# 	#17:lamb=300 t3 perf=70
# 	#16:lamb=310 t3 perf=73
# 	#15:lamb=285 t3 perf=87
# 	#14:lamb=340 t3 perf=67
# 	#13:lamb=452 t3 perf=41
# 	#12:lamb=226 t3 perf=87 #11:lamb=113 t3 perf=100 #10:lamb=56
# 	#9:1.0,1.0,0.3 @ lr=0.0003 #8:1.0,1.0,0.5 @ lr=0.0003 #7:1.0,1.0,0.6 @ lr=0.0003 #6:1.0,1.0,0.7 @ lr=0.0003 #5:1.0,1.0,0.8 @ lr=0.0003 #4:0.5,1.0,0.8 @ lr=0.0003 #3:0.1,1.0,0.75 #2:0.1,1.0,0.7 #1:0.1,1.0,0.8 #0:1.0,1.0,0.8
# 	found_best=false
# 	while [ $found_best=false ]
# 	do
# 		((alpha_lamb_i++))
...
# 	cur_pdm_frac=0.1
# 	alpha_lamb_i=8
# 	#8:0.9,1.0,0.1 lamb=56 t4 perf=57 (CF same as #1)
# 	#7:0.9,1.0,0.5 lamb=56 t4 perf=75 (CF same as #1)
# 	#6:0.9,1.0,0.8 lamb=80 t4 perf=65 (CF same as #1)
# 	#5:0.9,1.0,0.8 lamb=112 t4 perf=58 (CF same as #1)
# 	#4:0.9,1.0,0.8 lamb=56 t4 perf=76 (CF same as #1)
# 	#3:0.9,1.0,0.5 t4 perf=76 #2: 0.9,1.0,0.7 t4 perf=76 #1: 0.9,1.0,0.8 t4 perf=76
# 	found_best=false
# 	while [ $found_best=false ]
# 	do
# 		((alpha_lamb_i++))
...
do
	...
	cur_pdm_frac=1.0
	alpha_lamb_i=9
	#9:0.005,1.0,1.0 (lamb=56) t5 perf=60 (CF familiar to #8)
	#8:0.005,1.0,1.0 t5 perf=78
	#7:0.05,1.0,1.0
	#6:0.05,1.0,0.8
	found_best=false
	while [ $found_best=false ]
	do
		...
		printf "\n\nLA Phase\n\n"
		mkdir -p ${res_path}${id}.${best_lamb_i}.LA_phase.${alpha_lamb_i}/
		python  FABR//run.py --bert_model 'bert-base-uncased' --experiment hwu64 --approach bert_adapter_ewc_freeze --imp function --baseline ewc_freeze --backbone bert_adapter --note $note --idrandom $randid --seed $seed --scenario cil --use_rbs True --train_batch_size 128 --num_train_epochs 50 --valid_loss_es 0.002 --lr_patience 30 --custom_lr $custom_lr --custom_lamb $custom_lamb --fisher_combine avg --break_after_task $id --save_alpharel True --my_save_path ${res_path}${id}.${best_lamb_i}.LA_phase.${alpha_lamb_i}/ --start_at_task $id --start_model_path $start_model_path --elasticity_down_max_lamb $elasticity_up_max_lamb --elasticity_down_mult $elasticity_up_mult --elasticity_up $lamb_down --frel_cut_type pdm --pdm_frac $cur_pdm_frac --no_frel_cut_max $no_frel_cut_max --la_model_path $la_model_path --no_reg_in_LA True --la_num_train_epochs 7
		python3 FABR/calc_next_lamb_down_lamb_up.py --my_save_path ${res_path}${id} --rand_idx $randid --seed $seed --dataset $dataset --best_lr_id $best_lr_id --best_lamb_i $best_lamb_i --alpha_lamb_i $alpha_lamb_i --lamb_down $lamb_down --elasticity_up_mult $elasticity_up_mult --growth $growth --tid $id
		found_best=`cat ${res_path}${id}.${best_lamb_i}.LA_phase.${alpha_lamb_i}_foundbestlambdown.txt`
		python3 FABR/plot_lamb_down_results.py --my_save_path ${res_path}${id} --rand_idx $randid --seed $seed --dataset $dataset --best_lamb_i $best_lamb_i --alpha_lamb_i $alpha_lamb_i --lamb_down $lamb_down --tid $id
		# if [ $found_best = found ]; then
