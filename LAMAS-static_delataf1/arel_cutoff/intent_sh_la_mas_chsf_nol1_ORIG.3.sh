...
growth=0.1
res_path="/home/local/data/ms/fabr_data/IntentSH/IntentSH_LAMAS_NoL1_ORIG.3/${note}seed${seed}_${acc_drop_threshold}adt/IntentSH_LAMAS_t"
...
do
	...
	CUDA_VISIBLE_DEVICES=0 python  FABR//run.py --bert_model 'bert-base-uncased' --experiment hwu64 --approach bert_adapter_ewc_freeze --imp function --baseline ewc_freeze --backbone bert_adapter --note $note --idrandom $randid --seed $seed --scenario cil --use_rbs True --train_batch_size 128 --num_train_epochs 50 --valid_loss_es 0.002 --lr_patience 5 --custom_lr $custom_lr --custom_lamb $custom_lamb --fisher_combine max --break_after_task $id --save_alpharel True --my_save_path ${res_path}${id}.${best_lamb_i}.LA_phase/ --start_at_task $id --start_model_path $start_model_path --adapt_type orig --elasticity_down 100 --elasticity_up 0.01 --frel_cut 0.5
